...
namespace DDT {
    struct Codelet {
        ...
        // statemet (operation)
        Codelet(int br, int bc, int rw, int cw, int fnl, int ro, int co,
                int *offs)
            : lbr(br), row_width(rw), lbc(bc), col_width(cw),
              first_nnz_loc(fnl), row_offset(ro), col_offset(co), offsets(offs),
              multi_stmt(false) {}
        ...
        // Mostly for Type 3
        Codelet(int br, int bc, int rw, int cw, int fnl, int ro, int co,
                int *offs, int *offs2, int *offs3)
            : lbr(br), row_width(rw), lbc(bc), col_width(cw),
              first_nnz_loc(fnl), row_offset(ro), col_offset(co), offsets(offs),
              offsets2(offs2), offset3(offs3), multi_stmt(false) {}

        Codelet(int br, int bc, int rw, int cw, int fnl, int ro, int co,
                int *offs, int *offs2, int *offs3, bool ms)
            : lbr(br), row_width(rw), lbc(bc), col_width(cw),
              first_nnz_loc(fnl), row_offset(ro), col_offset(co), offsets(offs),
              offsets2(offs2), offset3(offs3), multi_stmt(ms) {}


        virtual ~Codelet() = default;

        virtual DDT::CodeletType get_type() = 0;
    ...
    struct FSCCodelet : public Codelet {
         ...
         */
        FSCCodelet(int br, int bc, int rw, int cw, int fnl, int ro, int co)
            : Codelet(br, bc, rw, cw, fnl, ro, co, NULL){};

        FSCCodelet(int br, int bc, int rw, int cw, int fnl, int ro, int co,
                   bool ms)
            : Codelet(br, bc, rw, cw, fnl, ro, co, NULL, ms){};
    ...
    struct PSCT1V1 : public Codelet {
        ...
        //int *row_offsets; //RO
        PSCT1V1(int br, int bc, int rw, int cw, int co, int *ros)
            : Codelet(br, bc, rw, cw, -1, -1, co, ros){};

        PSCT1V1(int br, int bc, int rw, int cw, int co, int *ros, bool ms)
            : Codelet(br, bc, rw, cw, -1, -1, co, ros, ms){};
    ...
    struct PSCT2V1 : public Codelet {
        ...
        PSCT2V1(int br, int rw, int cw, int fnl, int ro, int co, int *cio)
            : Codelet(br, -1, rw, cw, fnl, ro, co, cio){};
    ...
    struct PSCT3V1 : public Codelet {
        ...
        PSCT3V1(int rs, int nn, int fnl, int *cio)
            : Codelet(rs, -1, 1, nn, fnl, -1, 1, cio){};

        PSCT3V1(int rs, int nn, int fnl, int *cio, bool ms)
            : Codelet(rs, -1, 1, nn, fnl, -1, 1, cio, ms){};
    ...
    struct PSCT3V2 : public Codelet {
        ...
        PSCT3V2(int br, int bc, int rw, int cw, int co, int *ros, int *rls)
            : Codelet(br, -1, rw, cw, -1, -1, co, ros, rls, NULL){};
    ...
    struct PSCT3V3 : public Codelet {
        ...
        PSCT3V3(int br, int bc, int rw, int cw, int co, int *ros, int *rls,
                int *lbr)
            : Codelet(br, -1, rw, cw, -1, -1, co, ros, rls, lbr){};

        PSCT3V3(int br, int bc, int rw, int cw, int co, int *ros, int *rls,
                int *lbr, bool ms)
            : Codelet(br, bc, rw, cw, -1, -1, co, ros, rls, lbr, ms){};

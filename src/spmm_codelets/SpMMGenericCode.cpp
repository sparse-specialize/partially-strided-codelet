    ...
    void fsc_t2_2DC_gemm(double *Cx, const double *Ax, const double *Bx,
        ...
        for (int i = lb; i < ub - 1; i += 2) {
            for (int k = 0; k < cbb; k++) {
                ...
                int j = 0;
                for (; j < (cbu - cbl) - 3; j += 4) {
                    auto xv0 = _mm256_loadu_pd(x0 + cbd * k + j);
                    auto xv1 = _mm256_loadu_pd(x1 + cbd * k + j);
                ...
                // Compute tail
                __m128d tail = _mm_loadu_pd(Cx + i + cbd * k);
                for (; j < (cbu - cbl); j++) {
                    tail[0] += ax0[j] * x0[j + cbd * k];
                    tail[1] += ax1[j] * x1[j + cbd * k];
                }
            ...
            if (cr) {
                for (int k = 0; k < cbb; k++) {
                    ...
                    int j = 0;
                    for (; j < (cbu - cbl) - 3; j += 4) {
                        auto xv = _mm256_loadu_pd(x0 + j + cbd * k);
                        auto axv0 = _mm256_loadu_pd(ax0 + j);
                    ...
                    double tail = 0.;
                    for (; j < cbu - cbl; j++) {
                        tail += *(ax0 + j) * x0[j + cbd * k];
                    }

                    // H-Sum
                    Cx[ub - 1 + cbd * k] += tail + hsum_double_avx(r0);
                }
            }
        }
    }
        ...
        for (int i = lb, ii = 0; i < ub - tii; i += 4, ii += 4) {
            for (int kk = 0; kk < cbb; kk++) {
                ...
                for (int j = lbc, k = offset[ii], k1 = offset[ii + 1],
                    ...
                    x_reg.v = _mm256_loadu_pd((double *) (Bx + j + kk * cbd));
                ...
                Cx[i + kk * cbd] += (h0[0] + h0[2] + t0);
                Cx[i + 1 + kk * cbd] += (h0[1] + h0[3] + t1);
                h0 = _mm256_hadd_pd(result3.v, result4.v);
                Cx[i + 2 + kk * cbd] += (h0[0] + h0[2] + t2);
                Cx[i + 3 + kk * cbd] += (h0[1] + h0[3] + t3);
            }
        }
        /** the rest **/
...
*/
    void psc_t2_2DC_gemm(double *Cx, const double *Ax, const double *Bx,
        ...
        for (int i = lb; i < ub - co; i += 2) {
            for (int k = 0; k < cbb; k++) {
                ...
                for (; j < cb - 3; j += 4) {
                    __m256d xv0, xv1;
                    loadbx(offset, x0, j, xv0, msk0, cbd * k);
                    loadbx(offset, x1, j, xv1, msk1, cbd * k);
                ...
                for (; j < cb; j++) {
                    tail[0] += ax0[j] * x0[offset[j] + cbd * k];
                    tail[1] += ax1[j] * x1[offset[j] + cbd * k];
                }
        ...
        if (co) {
            for (int k = 0; k < cbb; k++) {
                ...
                int j = 0;
                for (; j < cb - 3; j += 4) {
                    loadbx(offset, x0, j, xv, msk, cbd * k);
                    auto axv0 = _mm256_loadu_pd(ax0 + j);
                    r0 = _mm256_fmadd_pd(axv0, xv, r0);
                }
    ...
    void psc_t3_1D1R_gemm(double *Cx, const double *Ax, const int *Ai,
        ...
        for (int k = 0; k < cbb; k++) {
            for (; j < cw - 3; j += 4, k += 4) {
                x_reg.v = _mm256_set_pd(Bx[offset[j + 3] + k * cbd],
                                        Bx[offset[j + 2] + k * cbd],
                                        Bx[offset[j + 1] + k * cbd],
                                        Bx[offset[j] + k * cbd]);
